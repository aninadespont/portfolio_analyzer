...
#This program lets the user build a custom stock portfolio and analyse its: returns, volatility, CAGR, beta, Sharpe ratio, correlation, diversification, CAPM, alpha vs a chosen benchmark, and a simple 1-year EMA-based forecast.
...
#This program is divided into multiple functions, each responsible for a specific task
#The functions are called throughout the program as needed
#All functions are executed at the end of the program
...
def get_benchmark_choice(period):
    while True:
        try:
            benchmark = int(input("\nWhich index would like as a benchmark:\n"
                "1. S&P 500\n"
                "2. Nasdaq\n"
                "3. Dow Jones\n"
                "\nEnter 1, 2, or 3: "))
...
def get_stock_metrics(portfolio, period, benchmark_price):
    ...
    #Cacluation of the average annual return over the chosen period to know how a typical year performs on average and identify the best and worst year
    ...
    df_stock_metrics = pd.DataFrame({
        ...
        "Period": period,
        "Average annual return (%)": (avg_annual_stock_return.values * 100),
        "Best year (%)": (best_year_stock_return.values * 100),
        "Worst year (%)": (worst_year_stock_return.values * 100),
        "Annual volatility (%)":(annual_stock_volatility.values * 100), 
        "CAGR (%)": (cagr_stocks.values * 100),
        "Beta": beta_stocks
            })
    ...
    return df_stock_metrics, daily_returns_stocks, yearly_stock_returns, avg_annual_stock_return, annual_stock_volatility, beta_stocks, prices
...
def get_portfolio_metrics(portfolio, daily_returns_stocks, yearly_stock_returns, avg_annual_stock_return, annual_stock_volatility, beta_stocks, prices, period):
    weights = [item["weight"] for item in portfolio]
    tickers = [item["ticker"] for item in portfolio]

    #Fetching the risk-free rate from the appropriate Treasury ticker for the selected period
    rfr_ticker = get_rfr_ticker(period)
    raw_rfr = yf.download(rfr_ticker, period="5d", auto_adjust=True, progress=False)["Close"]
    
    #Converting the last available risk free rate into decimal form to compute the sharpe ratio
    risk_free_rate = float((raw_rfr.iloc[-1].iloc[0]) / 100)
    ...
    #Portfolio volatility: accounts for the correlation between stocks via the covariance matrix
    ...
    correlation_matrix = daily_returns_stocks.corr().values
    n = len(tickers)
    weighted_avg_corr = 0
    ...
    #Returning all computed metrics in a table
...
#CAPM computation
def get_capm(portfolio, daily_returns_stocks, yearly_stock_returns, avg_annual_stock_return, annual_stock_volatility, beta_stocks, risk_free_rate, benchmark_name, yearly_benchmark_returns, benchmark_avg, avg_annual_return_portfolio, portfolio_beta):
...
#Securty Market Line calculation and graph creation 
def get_sml(benchmark_avg, beta_stocks, risk_free_rate, portfolio_beta, avg_annual_return_portfolio, portfolio_alpha):
...
#1-year (252 trading days) portfolio return forecast
#Using a weighted blend (50/50) of:
# - Exponential moving average: historical returns with exponentially decaying weights, placing more weight on the most recent 63 days (3 months) to capture recent market trends
# - Historical moving average: average daily returns over the historical period
#This combination captures recent trends while reducing overreaction to short-term noise
...
def get_ema_forecast(prices, portfolio, portfolio_prices, benchmark_price, benchmark_name, forecast_days=252):
    ...
    portfolio_ema_drift = float(portfolio_norm_daily_returns.ewm(span=63).mean().iloc[-1])
    portfolio_ema_volatility = float(portfolio_norm_daily_returns.ewm(span=63).std().iloc[-1])

    benchmark_ema_drift = float(benchmark_norm_daily_returns.ewm(span=63).mean().iloc[-1])
    benchmark_ema_volatility = float(benchmark_norm_daily_returns.ewm(span=63).std().iloc[-1])
    ...
    #Summarry table showing the projected 1Y return in percentage, the projected end price, and the annualized forecasted volatility in percentage for the benchmark and the portfolio
...
df_stock_metrics, daily_returns_stocks, yearly_stock_returns, avg_annual_stock_return, annual_stock_volatility, beta_stocks, prices = get_stock_metrics(portfolio, period, benchmark_price)

df_portfolio_metrics, risk_free_rate, avg_annual_return_portfolio, portfolio_beta, portfolio_prices = get_portfolio_metrics(portfolio, daily_returns_stocks, yearly_stock_returns, avg_annual_stock_return, annual_stock_volatility, beta_stocks, prices, period)
...
#Computation and display of the CAPM and SML analysis, table, and graph
capm_stock_df, capm_portfolio_df, stock_capm_returns, stock_alphas, portfolio_capm_return, portfolio_alpha = get_capm(portfolio, daily_returns_stocks, yearly_stock_returns, avg_annual_stock_return, annual_stock_volatility, beta_stocks, risk_free_rate, benchmark_name, yearly_benchmark_returns, benchmark_avg, avg_annual_return_portfolio, portfolio_beta)
...
get_sml(benchmark_avg, beta_stocks, risk_free_rate, portfolio_beta, avg_annual_return_portfolio, portfolio_alpha)

#Generation and display of the 1-year blended forecast graph and summary table 
df_forecast_summary = get_ema_forecast(prices, portfolio, portfolio_prices, benchmark_price, benchmark_name, forecast_days=252)

print("\n1-Year EMA Forecast Summary:\n", df_forecast_summary.round(3).to_string(index=False))
